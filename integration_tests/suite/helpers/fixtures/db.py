...
def chat_provider(**provider_args):
    def decorator(decorated):
        @wraps(decorated)
        def wrapper(self, *args, **kwargs):
            provider_args.setdefault('uuid', uuid.uuid4())
            provider_args.setdefault('tenant_uuid', TOKEN_TENANT_UUID)
            provider_args.setdefault('type_', 'sms')
            provider_args.setdefault('backend', 'twilio')
            provider_args.setdefault('name', f'provider-{_random_string(6)}')

            provider = ChatProvider(**provider_args)
            ...
            try:
                ...
                self._session.expunge_all()
                self._session.query(ChatProvider).filter(
                    ChatProvider.uuid == provider_args['uuid']
                ).delete()
                self._session.commit()
            return result
...
def user_alias(**alias_args):
    def decorator(decorated):
        @wraps(decorated)
        def wrapper(self, *args, **kwargs):
            alias_args.setdefault('uuid', uuid.uuid4())
            alias_args.setdefault('tenant_uuid', TOKEN_TENANT_UUID)
            alias_args.setdefault(
                'identity', f'+1555{random.randint(1000000, 9999999)}'
            )

            user_autogenerated = False
            if 'user_uuid' not in alias_args:
                user_uuid = uuid.uuid4()
                ...
                alias_args['user_uuid'] = user_uuid
                user_autogenerated = True

            provider_autogenerated = False
            if 'provider_uuid' not in alias_args:
                ...
                provider = ChatProvider(
                    ...
                    type_='sms',
                    backend='test',
                    name=f'auto-provider-{_random_string(6)}',
                )
                self._session.add(provider)
                self._session.flush()
                alias_args['provider_uuid'] = provider_uuid
                provider_autogenerated = True

            alias = UserAlias(**alias_args)
            ...
            try:
                ...
                self._session.expunge_all()
                self._session.query(UserAlias).filter(
                    UserAlias.uuid == alias_args['uuid']
                ).delete()
                if provider_autogenerated:
                    self._session.query(ChatProvider).filter(
                        ChatProvider.uuid == alias_args['provider_uuid']
                    ).delete()
                if user_autogenerated:
                    self._session.query(User).filter(
                        User.uuid == alias_args['user_uuid']
                    ).delete()
                self._session.commit()
            return result
